function [fr Resp]=PB_ClutterNew_SingleUnit(subjID,experiment,day,unitnumber,doplot)
if nargin < 5
    doplot = 1;
end

strctUnit = fnFindMat(subjID,day,experiment,unitnumber,'ClutteredFaceNew');
...
for i = 1:length(filename);
    ...
    if strcmpi(attribute,'noisebk')%||(attribute,'clutterbk')
        a2bStimulusToCondition(i,64) = 1;
    elseif strcmpi(attribute,'clutterbk')
        a2bStimulusToCondition(i,65) = 1;
        ...
        yy = (ynumber-50)/75+1;
        index = sc(xx,yy);
        a2bStimulusToCondition(i,3*(index-1)+k) = 1;
    end
end
...
Resp = fnAveragePB(strctUnit.m_a2bRaster_Valid, strctUnit.m_aiStimulusIndexValid, a2bStimulusToCondition, strctUnit.m_aiPeriStimulusRangeMS, 60, 220);
