...
[dd Unitlist] = ppdata_sil(subj,region);
...
for i = 1:length(dd)
    ...
    for j = 1:length(unitnumber)
        ...
        [ExpDate unitstr]
        [fr fr_rb tc] = PB_TransformRotation_SingleUnit(subj,'Test',ExpDate,unitstr,0);
        if ~isnan(fr)
            frall(:,k) = fr;
            fr_rball(:,k) = fr_rb;
            tcall(:,:,k) = tc;
...
save(['C:\PB\RotationImages\' subj '_Transform_population_' region '.mat'], 'finalresult');
