function alloutput = OcclusionAnalysis(subjID,experiment,foldername,unitnumber,avgwindowlength)
close all;
usePB = 1;
...
for timestart = 1:250
    ...
    if usePB
        [strctUnit.m_afAvgFirintRate_Stimulus, strctUnit.m_afAvgFirintRate_StimulusStd, aiCount] = fnAveragePB(strctUnit.m_a2bRaster_Valid,  strctUnit.m_aiStimulusIndexValid, diag(1:max(strctUnit.m_aiStimulusIndexValid))>0, strctUnit.m_aiPeriStimulusRangeMS, timestart, timestart+avgwindowlength);
    ...
    % figure;
    % % subplot(1,3,1);
    % % plot(snr,squeeze(mean(Single_Response(1:3,:,:))),'o-','linewidth',2);
    % title('face');
    % subplot(1,3,2);
    % plot(snr,squeeze(mean(Single_Response(4:6,:,:))),'o-','linewidth',2);
    % title('Object');
    % subplot(1,3,3);
    % plot(snr,squeeze((Single_Response(7,:,:))),'o-','linewidth',2);
    % title('Noise');
    % legend('Center','left','right');
    %
    % h1 = figure;
    % subplot(2,2,1);
    % plot(snr,squeeze(mean(Single_Response,1)),'o-','linewidth',2);
    % subplot(2,2,2);
    % plot(snr,squeeze(mean(Single_Response_MinusBaseline,1)),'o-','linewidth',2);
    % legend('Center','left','right');
    %
    ...
    %
    % subplot(2,2,3);
    % plot(snr,CategoryResponse,'o-','linewidth',2);
    % hold on;
    %
    % subplot(2,2,4);
    % plot(snr,CategoryResponse_MinusBaseline,'o-','linewidth',2);
    % legend('Face','Object','Noise');
    %
    % count = 1;
    %
    %
