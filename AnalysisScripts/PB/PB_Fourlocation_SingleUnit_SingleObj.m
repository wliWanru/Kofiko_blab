...
for i = 1:4
    for j = 1:4
        ...
        for cc = 10:20:90
            indexface= find(CondMatrix(:,2) == 0 & CondMatrix(:,3) == i & CondMatrix(:,4) == 0 & CondMatrix(:,5) == cc & CondMatrix(:,1) == 1);
            indexobj = find(CondMatrix(:,1) == 0 & CondMatrix(:,4) == j & CondMatrix(:,3) == 0 & CondMatrix(:,5) == cc & CondMatrix(:,2) == 2);
            indexcomb = find( CondMatrix(:,4) == j & CondMatrix(:,3) == i & CondMatrix(:,5) == cc & CondMatrix(:,2) == 2 & CondMatrix(:,1) == 1);
