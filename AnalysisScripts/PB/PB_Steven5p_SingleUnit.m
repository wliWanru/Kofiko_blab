...
strctUnit = fnFindMat(subjID,day,experiment,unitnumber,'Steven5objects');
...
condname = {'Center','Down','left','right','up'};
figure;
for i = 1:5
    for j = 1:5
        subplot(5,5,(i-1)*5+j);
        plot(fr(:,i),fr(:,j),'.');
        [tt p] = corrcoef(fr(:,i),fr(:,j));
        tt = tt(1,2);
        xlabel('TwoImage');
        ylabel('DownImage');
        title(['Coeffiency between' condname{i} ' and ' condname{j} ' is ' sprintf('%3.3g',tt)]);
        hold on;
