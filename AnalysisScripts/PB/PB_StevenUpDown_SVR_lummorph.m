...
load('D:\PB\stimuli\attachments\lumpp.mat');
pp = lummorph;
...
for cond = 1:4 % 1 both 2 down 3 up
    ...
    pred_pp_own = svm_reg(pp,bothdata,100);
...
save('D:\PB\StevenUpDown\Rocco_SVR_own_lummorph.mat','pred_pp_own_all');
% figure;
...
for traindata = 1:4
    for testdata = 1:4
        if ~(traindata == testdata)
            ...
            plab = svm_reg_tran(pp,trainresp,testresp);
            pred_pp_between_all(:,:,traindata,testdata) = plab;
        end
    end
end
save('D:\PB\StevenUpDown\Rocco_SVR_between_lummorph.mat','pred_pp_between_all');
% for t1 = 1:4
%     figure;
%     for cond = 1:4
