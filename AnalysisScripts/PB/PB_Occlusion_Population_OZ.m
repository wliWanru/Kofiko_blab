...
dd = {'150214','150216','150311'};
Unitlist{1} = [1 2 3 4 5 8 12 14 15];
Unitlist{2}= [3:26 28:30];
%Unitlist{3} = [1:5 9:10 18 24 26 28];
...
for i = 1:length(dd)
    ...
    for j = 1:length(unitnumber)
        ...
        [ExpDate unitstr]
        fr = PB_Occlusion_SingleUnit(Subject,[],ExpDate,unitstr,0)
        if ~isnan(fr)
...
save('D:\PB\OcclusionResult\Ozomatli_population.mat', 'finalresult');

return;

clear all;
load('D:\PB\OcclusionResult\Ozomatli_population.mat');
...
set(gcf,'Position',[ 153         410        1099         498]);
