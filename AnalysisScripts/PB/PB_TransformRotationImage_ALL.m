        ...
        if isdir(fn) && strcmp(fn(1),'1');
            ...
            if isdir('Processed');
                ...
                if isdir('SingleUnitDataEntries');
                    ...
                    zz = dir('*RotationImage_Transformed.mat');
                    for k = 1:length(zz)
                        mn = zz(k).name;
                        [fr fr_rb aiCount tc] = PB_TransformRotation_SingleUnit(mn);
                        tc = tc(:,1:900);
                        if ~isnan(fr)
                            frall(:,index) = fr;
                            fr_rball(:,index) = fr_rb;
                            aiCountall(:,index) = aiCount;
                            tcall(:,:,index) = tc; 
                            allsubj{index} = subjlist{i};
                            allexp{index} = Exp{i};
                            alldate{index} = fn;
                            underscoreindex = find(mn == '_');
                            allneuron{index} = mn(underscoreindex(8)+1:underscoreindex(9)-1)
                            index = index + 1;
                            index
                        end
                    end
...
cd('C:\PB\RotationImages');
save alltransform *all*
