...
dd = {'150508','150512','150515','150519','150521'};
Unitlist{1} = [9 10 11 16];
Unitlist{2} = [1 2 3 8 9];
Unitlist{3} = [4 5 6 7 8 9 10 11 13 15 18 19 20];
Unitlist{4} = [11 12 32];
Unitlist{5} = [4 5 6 8 9 10 11 13 22];
...
for i = 1:length(dd)
    ...
    for j = 1:length(unitnumber)
        ...
        [ExpDate unitstr]
        output = PB_Stevenlr_SingleUnit(Subject,'test',ExpDate,unitstr)
...
save('D:\PB\Stevenlr\Houdini_AL_population.mat', 'finalresult');

return;

load('D:\PB\Stevenlr\Rocco_population.mat');
...
plaball = reshape(plaball,3000,1);
for i = 1:3
    ss = find(y == i)
    for j = 1:3
        index = find(plaball(ss)==j);
        rr(i,j) = length(index);
    end
end
