...
dd = {'150420','150423'};
Unitlist{1} = [6:8];
Unitlist{2} = [1 2 4 5 7 8 9 14 15];
Subject = 'Houdini';
...
for i = 1:length(dd)
    ...
    for j = 1:length(unitnumber)
        ...
        [ExpDate unitstr]
        output = PB_TwoImageUpDown_SingleUnit(Subject,'test',ExpDate,unitstr,0)
        if ~isnan(output)
...
save('D:\PB\TwoImageResult\Houdini_TwoImageUpDown_population.mat', 'finalresult');

return;
close all;
clear all;
load('D:\PB\TwoImageResult\Houdini_TwoImageUpDown_population.mat');
