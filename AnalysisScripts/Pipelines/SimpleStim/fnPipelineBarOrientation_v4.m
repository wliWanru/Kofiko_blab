...
fnWorkerLog('Starting simple stim standard analysis pipline...');
...
strConfigFile = [strConfigFolder,'AnalysisPipelines',filesep, 'PipelineSimpleStim.xml'];
...
if ~ismember('Simple Stim',acstrParadigmNames)
    fnWorkerLog('Session : %s does not contain simple stim. Aborting!',strSession);
    return;
end;
...
for iChannelIter=1:iNumSortedChannels
    ...
    for iUnitIter=1:iNumSortedUnits
        ...
        if ismember('Simple Stim',acParadigmsRecorded)
            ...
            acUnitsStat = fnCollectSimpleStimUnitStats2(strctKofiko, strctSync, strctConfig, strctInterval,afActualFlipTime_PLX);
            if ~isempty(acUnitsStat)
                ...
                for iEntryIter=1:iNumDataEntries
                    ...
                    strDesr(strDesr == ' ') = '_';
                    if strcmp(strDesr(1),'\');
                        [junk strDesr] = fileparts(strDesr);
                    end
