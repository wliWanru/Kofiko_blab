function ahSubPlots = fnDisplaySimpleStim(ahPanels,strctUnit)
hParent = ahPanels(1);
...
%% ~~ recording depth range
if isfield(strctUnit,'m_afRecordingRange') && ~isempty(strctUnit.m_afRecordingRange)
    set(hParent,'Title', ...
        sprintf( 'Recording Depth: %.2f - %.2f', ...
        strctUnit.m_afRecordingRange(1), ...
        strctUnit.m_afRecordingRange(2)));
else
    set(hParent,'Title', 'Recording Depth: Unknown');
end

%% ~~ raster plot upper left; trial-by-trial with time
h1 = tightsubplot(3,3,1,'Spacing',0.1,'Parent',hParent);
% ~~ imagesc(x_timepoints, y_stimulusIndex, color_firingRate) 
...
%% ~~ raster plot upper middle; category-averaged with time
...
% ~~~ plot white color annotation line of 0 ms, stimON and stim OFF ~~~
% ~~~~~ ymin=0.5, ymax=6.5 because min cate num is 1, max cate num is 6
fON = median(strctUnit.m_strctStimulusParams.m_afStimulusON_MS);  % ~~ here is 250 ms
fOFF = fON + median(strctUnit.m_strctStimulusParams.m_afStimulusOFF_MS);  % 250 + 150 = 400 ms
...
if ~isfield(strctUnit.m_strctStatParams,'m_iStartAvgMS')
    plot([strctUnit.m_strctStatParams.m_fStartAvgMS, strctUnit.m_strctStatParams.m_fStartAvgMS], ...
        [0.5, 0.5+size(strctUnit.m_a2fAvgFirintRate_Category,1)],'r')
    plot([strctUnit.m_strctStatParams.m_fEndAvgMS, strctUnit.m_strctStatParams.m_fEndAvgMS], ...
        [0.5, 0.5+size(strctUnit.m_a2fAvgFirintRate_Category,1)],'r')
else
    % ~~~ ??? where did we get m_iStartAvgMS ???
    % ~~~ is it the averaged time of the detected spike on and off??????
    % ~~~ plot red color annotation line of averaged spike ON and spike OFF ~~~
    plot([strctUnit.m_strctStatParams.m_iStartAvgMS, strctUnit.m_strctStatParams.m_iStartAvgMS], ...
        [0.5, 0.5+size(strctUnit.m_a2fAvgFirintRate_Category,1)],'r')
    plot([strctUnit.m_strctStatParams.m_iEndAvgMS, strctUnit.m_strctStatParams.m_iEndAvgMS], ...
        [0.5, 0.5+size(strctUnit.m_a2fAvgFirintRate_Category,1)],'r')
end
...
%% ~~ right bar plot, avg firing and significance
h3=tightsubplot(1,3,3,'Spacing',0.13,'Parent',hParent);
hold off;

hold on;
% plot(h3,[strctUnit.m_fAvgBaseline strctUnit.m_fAvgBaseline], ...
%     [0, strctUnit.m_iNumCategories+0.5],'c','Linewidth',2);
% axis([floor(min(strctUnit.m_afAvgFiringSamplesCategory)), ...
%     eps+ceil(max(strctUnit.m_fAvgBaseline,max(strctUnit.m_afAvgFiringSamplesCategory))) ...
%     0 strctUnit.m_iNumCategories+0.5])

barh(1:strctUnit.m_iNumCategories, strctUnit.m_afAvgFiringSamplesCategory,0.7);
% ~~ strctUnit.m_acConditionNames is category {'Faces'}    {'Bodies'} .. names
% ~~ add corresponding index to these categories and set yticklabels
acCatNamesAug = cell(1,length(strctUnit.m_acConditionNames));
for k=1:length(acCatNamesAug)
    acCatNamesAug{k} = sprintf('%s [%d]',strctUnit.m_acConditionNames{k},k);
end
set(h3,'ytick',1:length(strctUnit.m_acConditionNames), 'YtickLabel',acCatNamesAug)

% ~~ add significance marker ~~~
% ~~ the significant stats strctUnit.m_a2fPValueCat seems like every paired t test results
fSignificanceLevel = 0.01;
if isfield(strctUnit,'m_a2fPValueCat')
    % ~~ select all non-NaN rows of the last column ~~
    abHypothesisRejected = strctUnit.m_a2fPValueCat(1:end-1,end) < fSignificanceLevel;
    plot(h3,1+strctUnit.m_afAvgFiringSamplesCategory(abHypothesisRejected), ...
        find(abHypothesisRejected), 'r*');
end

if ~isfield(strctUnit.m_strctStatParams,'m_iStartAvgMS')
    xlabel( sprintf('Avg firing [%d, %d] ms', ...
        strctUnit.m_strctStatParams.m_fStartAvgMS,strctUnit.m_strctStatParams.m_fEndAvgMS))
else
    xlabel( sprintf('Avg firing [%d, %d] ms', ...
        strctUnit.m_strctStatParams.m_iStartAvgMS,strctUnit.m_strctStatParams.m_iEndAvgMS))
end
title('Avg Firing and significance');


%% ~~ second row left, avg spike wave form
...
if isfield(strctUnit,'m_a2fAvgWaveFormCat')
    ...
    afS = strctUnit.m_afStdWaveForm;  % ~~ shade errorbar
...
% ~~????
fFractionISIinvalid = sum( ...
    diff(strctUnit.m_afSpikeTimes) < 1*1e-3) / length(strctUnit.m_afSpikeTimes) * 1e2;
...
if isfield(strctUnit,'m_a2fAvgLFPCategory') && ~isempty(strctUnit.m_a2fAvgLFPCategory)
    % ~~ strctUnit.m_aiPeriStimulusRangeMS [-200, 900] timepoints
    % ~~ strctUnit.m_a2fAvgLFPCategory' (1100, 6), ERP for 6 cates
...
%% ~~ each stimulus picture presented times 
h7 = tightsubplot(3,3,7,'Spacing',0.13,'Parent',hParent);

if isfield(strctUnit,'m_a2bStimulusToCondition')
    iNumStimuli = size(strctUnit.m_a2bStimulusToCondition,1);
else
...
%% ~~ average firing rates at each timepoint????? how come????
h8 = tightsubplot(3,3,8,'Spacing',0.13,'Parent',hParent);
if isfield(strctUnit,'m_a2bStimulusToCondition')
    iNumCategories = size(strctUnit.m_a2bStimulusToCondition,2);
else
...
% colormap jet
% colorbar
% set(h9,'ytick',1:(length(strctUnit.m_acConditionNames)+1),...
% 'YtickLabel',[strctUnit.m_acConditionNames,'Baseline'])
% set(h9,'xtick',1:length(strctUnit.m_acConditionNames)+1);
%, 'XtickLabel',[strctUnit.m_acConditionNames,'Baseline'])
% title('Log10 p-value');
...
% colormap jet
% set(h10,'ytick',1:(length(strctUnit.m_acConditionNames)+1), 'YtickLabel',[strctUnit.m_acConditionNames,'Baseline'])
% set(h10,'xtick',1:length(strctUnit.m_acConditionNames)+1);%, 'XtickLabel',[strctUnit.m_acConditionNames,'Baseline'])
...
%     iFaceGroup = find(ismember(acUnits{aiFOB(iExpIter)}.m_acConditionNames,'Faces'));
%     aiNonFaceGroups = find(ismember(acUnits{aiFOB(iExpIter)}.m_acConditionNames,    {'Bodies',    'Fruits',    'Gadgets',    'Hands',    'Scrambles'}));
...
% legend([acUnits{aiFOB(1)}.m_acConditionNames(1:6),'Sinha','Sinha Scrambled','Pink Noise']);
% xlabel('Time (ms)');
