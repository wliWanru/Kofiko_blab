...
load(strSyncFile);
% strctSync.m_strctKofikoToPlexon.m_afOffset = strctSync.m_strctKofikoToPlexon.m_afOffset(end);
% strctSync.m_strctKofikoToPlexon.m_afScale = strctSync.m_strctKofikoToPlexon.m_afScale(end);
% strctSync.m_strctKofikoToPlexon.m_afMinJitter = strctSync.m_strctKofikoToPlexon.m_afMinJitter(end);
% strctSync.m_strctKofikoToPlexon.m_afMaxJitter = strctSync.m_strctKofikoToPlexon.m_afMaxJitter(end);
% strctSync.m_strctKofikoToPlexon.m_afMeanJitter = strctSync.m_strctKofikoToPlexon.m_afMeanJitter(end);
% strctSync.m_strctKofikoToPlexon.m_afStartFrameTS_PLX = strctSync.m_strctKofikoToPlexon.m_afStartFrameTS_PLX(end);
% strctSync.m_strctKofikoToPlexon.m_afEndFrameTS_PLX = strctSync.m_strctKofikoToPlexon.m_afEndFrameTS_PLX(end);
% strctSync.m_strctKofikoToPlexon.m_afStartFrameTS_Kofiko = strctSync.m_strctKofikoToPlexon.m_afStartFrameTS_Kofiko(end);
% strctSync.m_strctKofikoToPlexon.m_afEndFrameTS_Kofiko = strctSync.m_strctKofikoToPlexon.m_afEndFrameTS_Kofiko(end);
...
a2fTemp = textread('C:\PB\EphysData\Alfie\Test\160805\RAW\160805_152007_Alfie-Advancers.txt');
        ...
        if ismember('Passive Fixation New',acParadigmsRecorded)
            ...
            if ~isempty(acUnitsStat)
                ...
                for iEntryIter=1:iNumDataEntries
                    ...
                    strDesr(strDesr == ' ') = '_';
                    if strcmp(strctUnit.m_strImageListUsed,'\\192.168.50.15\StimulusSet\BOSOcclusion\BOSMemory.xml')
                        strDesr = 'BOSOcclusion';
                    end
