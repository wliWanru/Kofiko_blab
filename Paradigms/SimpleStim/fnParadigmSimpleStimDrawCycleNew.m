...
if ~((g_strctDraw.strctDesign.CurrentParameter(Index).ImageKind == 5)||(g_strctDraw.strctDesign.CurrentParameter(Index).ImageKind == 7))
    if g_strctDraw.strctDesign.CurrentParameter(Index).ImageKind == 6
        if isfield(g_strctDraw,'CurrentTexture');
            Screen('Close',g_strctDraw.CurrentTexture);
        end
        NoiseImage = g_strctDraw.strctDesign.Image{Index};
        g_strctDraw.CurrentTexture  = Screen('MakeTexture', g_strctPTB.m_hWindow,uint8(NoiseImage));
    else
    ...
    switch g_strctDraw.strctDesign.CurrentParameter(Index).ImageKind
        case 1
            ...
            PixelPerFrame =  OneCyclePixel*TF_HZ/g_strctPTB.m_iRefreshRate;
    ...
    offset = 0;
    if g_strctDraw.strctDesign.CurrentParameter(Index).ImageKind == 6
        Screen('DrawTexture', g_strctPTB.m_hWindow, g_strctDraw.CurrentTexture,[],StimRect,g_strctDraw.m_strctTrial.m_Orientation,0);
    else
    ...
    % Draw Fixation spot
    Screen('FillArc',g_strctPTB.m_hWindow,g_strctDraw.m_strctTrial.m_FixationColor , aiFixationRect,0,360);
...
elseif g_strctDraw.strctDesign.CurrentParameter(Index).ImageKind == 5
...
elseif g_strctDraw.strctDesign.CurrentParameter(Index).ImageKind == 7
    ...
    %s1_center = 0 + StimCenter; s2_center = 100 + StimCenter;

    Image_Size = g_strctDraw.strctDesign.Image_Size;
    SquareRadius = Image_Size(2);
    Screen('FillRect',g_strctPTB.m_hWindow,s1_color,[s1_center(1)-SquareRadius s1_center(2)-SquareRadius s1_center(1)+SquareRadius s1_center(2)+SquareRadius]);
    Screen('FillRect',g_strctPTB.m_hWindow,s2_color,[s2_center(1)-SquareRadius s2_center(2)-SquareRadius s2_center(1)+SquareRadius s2_center(2)+SquareRadius]);
    Screen('FillArc',g_strctPTB.m_hWindow,g_strctDraw.m_strctTrial.m_FixationColor , aiFixationRect,0,360);
    g_strctServerCycle.m_fLastFlipTime = fnFlipWrapper(g_strctPTB.m_hWindow); % This would block the server until the next flip.
    g_strctDraw.CurrentFrameIndex = 1; % This would block the server until the next flip.
...
function fnWaitNoiseImageONPeriod()
global g_strctDraw g_strctPTB g_strctServerCycle
fCurrTime  = GetSecs();
Index = g_strctDraw.m_strctTrial.Image_index;
