...
function fnParadigmSimpleStimCallbacksNew(strCallback,varargin)
%
...
switch strCallback
    case 'NanoStimulatorNumTrainRepeat'
    case 'NanoStimulatorInterTrainInterval'
    case 'NanoStimulatorSpikeTriggerCount'
        
    case 'MicroStimFrequencySweep'
        g_strctParadigm.m_strctNanoStimulator.m_iMachineState = 1;
    case 'MicroStimDelaySweep'
        g_strctParadigm.m_strctNanoStimulator.m_iMachineState = 10;
    case 'MicroStimFrequencySweepOptical'
        g_strctParadigm.m_strctNanoStimulator.m_iMachineState = 20;
        
    case 'MicroStimStopSweep'
    ...
    case 'CopyRFParameter'
        StimulusPos = g_strctParadigm.StimulusPos.Buffer(1,:,g_strctParadigm.StimulusPos.BufferIdx);
        ...
        set(g_strctParadigm.m_strctControllers.CenterX_Edit,'String',num2str(StimulusPos(1)));
        set(g_strctParadigm.m_strctControllers.CenterY_Edit,'String',num2str(StimulusPos(2)));
        StimulusSizePix = g_strctParadigm.StimulusSizePix.Buffer(1,:,g_strctParadigm.StimulusSizePix.BufferIdx);
        set(g_strctParadigm.m_strctControllers.Width_Edit,'String',num2str(StimulusSizePix(1)));
        set(g_strctParadigm.m_strctControllers.Length_Edit,'String',num2str(StimulusSizePix(1)));
        Orientation = g_strctParadigm.Orientation.Buffer(1,:,g_strctParadigm.Orientation.BufferIdx);
        set(g_strctParadigm.m_strctControllers.Orientation_Edit,'String',num2str(Orientation));
    ...
    case 'Random'
        ...
        if isfield(g_strctParadigm,'m_strctDesign') && (~isempty(g_strctParadigm.m_strctDesign))
            iNumMediaInBlock = length(g_strctParadigm.m_strctDesign.Image);
            if g_strctParadigm.m_bRandom
                [fDummy,g_strctParadigm.m_aiCurrentRandIndices_Image1] = sort(rand(1,iNumMediaInBlock));
                [fDummy,g_strctParadigm.m_aiCurrentRandIndices_Image2] = sort(rand(1,iNumMediaInBlock));
    ...
    case 'FixationColor'
        ...
        fnShowHideWind('PTB Onscreen window [10]:','show');
        if length(aiColor) > 1
            fnTsSetVarParadigm('FixationColor',round(aiColor*255));
            %            fnDAQWrapper('StrobeWord', fnFindCode('Stimulus Position Changed'));
        end;
        if bPausing
            fnResumeParadigm();
    ...
    case 'LoadFavoriteListSafe'
        ...
        if ~fnMakeImage(g_strctParadigm.m_acFavroiteLists{iSelectedImageList});
            return;
        end
        
        g_strctParadigm.m_CurrentDisplayIndex = 0;
        tt = GetSecs;
        [fLocalTime, fServerTime, fJitter] = fnSyncClockWithStimulusServer(100);
        ['Sync' num2str(GetSecs-tt)]
        fnTsSetVarParadigm('SyncTime', [fLocalTime,fServerTime,fJitter]);
        tt = GetSecs;
        fnResetStat();
        ['fnResetStat' num2str(GetSecs-tt)]
        ...
        g_strctParadigm.m_iCurrentMediaIndexInBlockList = 0;
    ...
    case 'RandFixationSpot'
        ...
        if g_strctParadigm.m_bRandFixPos
            ...
            if g_strctParadigm.m_bRandFixSyncStimulus
                fnTsSetVarParadigm('StimulusPos_Image1', [g_strctStimulusServer.m_aiScreenSize(3)/2-100 g_strctStimulusServer.m_aiScreenSize(4)/2]);
                fnTsSetVarParadigm('StimulusPos_Image2', [g_strctStimulusServer.m_aiScreenSize(3)/2+100 g_strctStimulusServer.m_aiScreenSize(4)/2]);
    ...
    case 'ParameterSweep'
        ...
        if (g_strctParadigm.m_bParameterSweep)
            ...
            g_strctParadigm.m_strctSavedParam.m_fON= fnTsGetVar(g_strctParadigm,'StimulusON_MS');
            g_strctParadigm.m_strctSavedParam.m_fOFF = fnTsGetVar(g_strctParadigm,'StimulusOFF_MS');
            ...
            fnTsSetVarParadigm('StimulusPos_Image1',g_strctParadigm.m_strctSavedParam.m_pt2fStimulusPosition_Image1);
            fnTsSetVarParadigm('StimulusPos_Image2',g_strctParadigm.m_strctSavedParam.m_pt2fStimulusPosition_Image2);
            ...
            fnTsSetVarParadigm('StimulusON_MS',g_strctParadigm.m_strctSavedParam.m_fON);
            fnTsSetVarParadigm('StimulusOFF_MS',g_strctParadigm.m_strctSavedParam.m_fOFF);
    ...
    case 'MotionFinished'
        if ~fnParadigmToKofikoComm('IsPaused')
            g_strctParadigm.m_strctCurrentTrial = fnTwoImagePrepareTrial();
            g_strctParadigm.m_iMachineState = 1;
        end
    ...
    case 'DrawAttentionEvent'
        g_strctParadigm.m_iMachineState = 1;
    case 'DrawRF'
        g_strctParadigm.m_bDrawRF = get(g_strctParadigm.m_strctControllers.m_hDrawRF,'value');
    case 'ClearRF'
    ...
    case 'QueryRF'
        ...
        RFcenter = fnParadigmToStatServerComm('QueryRF', g_strctParadigm.m_strctStatServerDesign);
        if RFcenter
            RFcenter
            StimulusPos = g_strctParadigm.StimulusPos.Buffer(1,:,g_strctParadigm.StimulusPos.BufferIdx);
            NewPos = StimulusPos + RFcenter;
            StimulusPos
            RFcenter
            fnTsSetVarParadigm('StimulusPos',NewPos);
            
            
        else
            fnLog('Error: Could not find STA Fit from StatServer.')
        end
...
fnParadigmToStimulusServer('PauseButRecvCommands');
fnHidePTB();
[strFile, strPath] = uigetfile([g_strctParadigm.m_strInitial_DefaultImageFolder,'*.m']);

fnShowPTB()
if strFile(1) ~= 0
    g_strctParadigm.m_strNextImageList = [strPath,strFile];
    
    if ~fnMakeImage(g_strctParadigm.m_strNextImageList);
        return;
    end;
