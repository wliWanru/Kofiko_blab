...
g_strctParadigm.ViewDistanceCM = 57;
g_strctParadigm.Width_ScreenCM = 38;
g_strctParadigm.PixelPerDegree = g_strctStimulusServer.m_aiScreenSize(4)/((atan((g_strctParadigm.Width_ScreenCM/2)/g_strctParadigm.ViewDistanceCM)*2/pi*180));

iSmallBuffer = 500;
iLargeBuffer = 50000;

% g_strctParadigm.m_strctNanoStimulator.m_iNumRepeats =10;
% g_strctParadigm.m_strctNanoStimulator.m_fInterTrainTimeSec = 5;
% g_strctParadigm.m_strctNanoStimulator.m_iNanoStimulatorSpikeTriggerCount = 30;
% g_strctParadigm = fnTsAddVar(g_strctParadigm, 'NanoStimulatorNumTrainRepeat', g_strctParadigm.m_strctNanoStimulator.m_iNumRepeats, iSmallBuffer);
% g_strctParadigm = fnTsAddVar(g_strctParadigm, 'NanoStimulatorInterTrainInterval', g_strctParadigm.m_strctNanoStimulator.m_fInterTrainTimeSec, iSmallBuffer);
% g_strctParadigm = fnTsAddVar(g_strctParadigm, 'NanoStimulatorSpikeTriggerCount', g_strctParadigm.m_strctNanoStimulator.m_iNanoStimulatorSpikeTriggerCount, iSmallBuffer);
% 
...
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'FixationColor',  [255 255 255], iSmallBuffer);
...
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'StimulusPos', [g_strctStimulusServer.m_aiScreenSize(3)/2 g_strctStimulusServer.m_aiScreenSize(4)/2] , iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'Contrast', 1, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'ImageKind', 1, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'BarWidth', 20, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'SurfaceColor', 255, iSmallBuffer);


g_strctParadigm = fnTsAddVar(g_strctParadigm, 'SF_CyclePerDeg', 0.6, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'TF_HZ', 2, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'Orientation', 0, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'StimulusSizePix_X', 100, iSmallBuffer); % propotion of the original Image1; Will calculte the Image size in the callback;
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'StimulusSizePix_Y', 100, iSmallBuffer); % propotion of the original Image1; Will calculte the Image size in the callback;
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'Depth', 5, iSmallBuffer); % propotion of the original Image1; Will calculte the Image size in the callback;

%g_strctParadigm = fnTsAddVar(g_strctParadigm, 'StimulusON_MS', g_strctParadigm.m_fInitial_StimulusON_MS, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'StimulusON_MS', 1000, iSmallBuffer);
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'StimulusOFF_MS', g_strctParadigm.m_fInitial_StimulusOFF_MS, iSmallBuffer);


g_strctParadigm.m_bRandFixPos = 0;
...
% g_strctParadigm.m_strctSavedParam.m_pt2fStimulusPosition_Image1 = fnTsGetVar(g_strctParadigm,'StimulusPos_Image1');
% g_strctParadigm.m_strctSavedParam.m_pt2fStimulusPosition_Image2 = fnTsGetVar(g_strctParadigm,'StimulusPos_Image2');
% 
...
g_strctParadigm = fnTsAddVar(g_strctParadigm, 'Trials',zeros(7,1),iLargeBuffer);
...
% g_strctParadigm.m_aiCurrentRandIndices_Image1 = [];
% g_strctParadigm.m_aiCurrentRandIndices_Image2 = [];
% 
% 
...
g_strctParadigm.m_bDrawRF = false;
g_strctParadigm.m_ToDrawRF = [];
...
% g_strctParadigm.m_astrctParameterSweepModes(1).m_afStimulusON  = [];
% g_strctParadigm.m_astrctParameterSweepModes(1).m_afStimulusOFF  = [];
% 
% g_strctParadigm.m_astrctParameterSweepModes(2).m_strName = 'ON/OFF Times';
% g_strctParadigm.m_astrctParameterSweepModes(2).m_afX  = 0;
% g_strctParadigm.m_astrctParameterSweepModes(2).m_afY  = 0;
% g_strctParadigm.m_astrctParameterSweepModes(2).m_afSize  = [];
% g_strctParadigm.m_astrctParameterSweepModes(2).m_afTheta  = [];
% g_strctParadigm.m_astrctParameterSweepModes(2).m_afStimulusON  = [100,200,500,1000];
% g_strctParadigm.m_astrctParameterSweepModes(2).m_afStimulusOFF  = [150,500];
% 
% 
% g_strctParadigm.m_astrctParameterSweepModes(3).m_strName = '7x7 Position Only';
...
% g_strctParadigm.m_astrctParameterSweepModes(3).m_afSize  = [];
% g_strctParadigm.m_astrctParameterSweepModes(3).m_afTheta  = [];
% g_strctParadigm.m_astrctParameterSweepModes(3).m_afStimulusON  = [];
% g_strctParadigm.m_astrctParameterSweepModes(3).m_afStimulusOFF  = [];
% 
% g_strctParadigm.m_astrctParameterSweepModes(4).m_strName = '7x7x3 Position & Scale';
% g_strctParadigm.m_astrctParameterSweepModes(4).m_afX  = [-300:100:300];
% g_strctParadigm.m_astrctParameterSweepModes(4).m_afY  = [-300:100:300];
% g_strctParadigm.m_astrctParameterSweepModes(4).m_afSize  = [32 64 128];
% g_strctParadigm.m_astrctParameterSweepModes(4).m_afTheta  = [];
% g_strctParadigm.m_astrctParameterSweepModes(4).m_afStimulusON  = [];
% g_strctParadigm.m_astrctParameterSweepModes(4).m_afStimulusOFF  = [];
% 
% g_strctParadigm.m_astrctParameterSweepModes(5).m_strName = '21x21 Position Only';
...
% g_strctParadigm.m_astrctParameterSweepModes(5).m_afSize  = [];
% g_strctParadigm.m_astrctParameterSweepModes(5).m_afTheta  = [];
% g_strctParadigm.m_astrctParameterSweepModes(5).m_afStimulusON  = [];
% g_strctParadigm.m_astrctParameterSweepModes(5).m_afStimulusOFF  = [];
% 
% g_strctParadigm.m_astrctParameterSweepModes(6).m_strName = '21x21x3 Position Only';
% g_strctParadigm.m_astrctParameterSweepModes(6).m_afX  = -400:40:400;
% g_strctParadigm.m_astrctParameterSweepModes(6).m_afY  = -300:30:300;
% g_strctParadigm.m_astrctParameterSweepModes(6).m_afSize  = [32 64 128];
% g_strctParadigm.m_astrctParameterSweepModes(6).m_afTheta  = [];
% g_strctParadigm.m_astrctParameterSweepModes(6).m_afStimulusON  = [];
% g_strctParadigm.m_astrctParameterSweepModes(6).m_afStimulusOFF  = [];
% 
