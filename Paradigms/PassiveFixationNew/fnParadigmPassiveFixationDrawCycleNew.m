...
% Draw Fixation spot
Screen('FillArc',g_strctPTB.m_hWindow,g_strctDraw.m_strctTrial.m_afFixationColor, aiFixationRect,0,360);
...
function fnWaitMonocularMovieOFFPeriod
global g_strctDraw g_strctPTB g_strctServerCycle
fCurrTime  = GetSecs();

if (fCurrTime - g_strctServerCycle.m_fLastFlipTime) > ...
        (g_strctDraw.m_strctTrial.m_fStimulusOFF_MS)/1e3 - (0.2 * (1/g_strctPTB.m_iRefreshRate) )
    fnStimulusServerToKofikoParadigm('TrialFinished',g_strctDraw.m_a2fFrameFlipTS,g_strctServerCycle.m_fLastFlipTime );
    g_strctServerCycle.m_iMachineState = 0;
end
return;
...
if g_strctDraw.m_strctTrial.m_bShowPhotodiodeRect
    Screen('FillRect',g_strctPTB.m_hWindow,g_strctDraw.m_strctTrial.m_afFixationColor, ...
        [g_strctPTB.m_aiRect(3)-g_strctDraw.m_strctTrial.m_iPhotoDiodeWindowPix ...
        g_strctPTB.m_aiRect(4)-g_strctDraw.m_strctTrial.m_iPhotoDiodeWindowPix ...
        g_strctPTB.m_aiRect(3) g_strctPTB.m_aiRect(4)]);
end
...
fLastFlipTime = fnMovieFlipWrapper(g_strctPTB.m_hWindow,1); % This would block the server until the next flip.
...
if hFrameTexture == -1
    ...
    % clear the last frame
        ...
        g_strctDraw.m_strctTrial.m_fStimulusOFF_MS
    if g_strctDraw.m_strctTrial.m_fStimulusOFF_MS > 0
        Screen('FillRect',g_strctPTB.m_hWindow, g_strctDraw.m_strctTrial.m_afBackgroundColor);
        ...
        g_strctServerCycle.m_fLastFlipTime = fnMovieFlipWrapper(g_strctPTB.m_hWindow,0);  % Block (!)
        fnStimulusServerToKofikoParadigm('FlipOFF',g_strctServerCycle.m_fLastFlipTime);
    ...
    Screen('FillArc',g_strctPTB.m_hWindow,g_strctDraw.m_strctTrial.m_afFixationColor, aiFixationRect,0,360);
    fLastFlipTime = fnMovieFlipWrapper(g_strctPTB.m_hWindow,0);  % Block (!)
...
%     else
        ...
        fLastFlipTime = fnMovieFlipWrapper(g_strctPTB.m_hWindow,1, g_strctDraw.m_fMovieOnset+fTimeToFlip);  % Block (!)
        Screen('Close', hFrameTexture);
...
return;
function fFlipTime = fnMovieFlipWrapper(hWindow,flag,varargin)
