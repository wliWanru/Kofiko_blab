...
strctTrial.m_afFixationColor = squeeze(g_strctParadigm.FixationColor.Buffer(1,:,g_strctParadigm.FixationColor.BufferIdx));
...
if ~isempty(iSelectedBlock) && g_strctParadigm.m_strctDesign.m_strctBlocksAndOrder.m_astrctBlocks(iSelectedBlock).m_bMicroStim
   ...
   iNumFoundAttributes = length(intersect(g_strctParadigm.m_strctDesign.m_strctBlocksAndOrder.m_astrctBlocks(iSelectedBlock).m_acMicroStimAttributes,...
    ...
    % TODO: Add micro stim preset and parameters here... Pinglei Add
...
%Added By Pinglei to add microstim in different timing 
strctMicroStim.m_MicrostimDelayMS = fnTsGetVar(g_strctParadigm,'MicrostimDelayMS');

for i = 1:length(lower(g_strctParadigm.m_strctDesign.m_astrctMedia(iNewStimulusIndex).m_acAttributes));
    attr = lower(g_strctParadigm.m_strctDesign.m_astrctMedia(iNewStimulusIndex).m_acAttributes{i});
    if ismember('microstim_delay',attr);
        strctMicroStim.m_MicrostimDelayMS = str2num(attr(length('microstim_delay')+1:end));
    end
end
